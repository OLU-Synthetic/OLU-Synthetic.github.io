(() => {
  ...
  if (grid) {
    grid.innerHTML = WORKS.map(w => `
      <a class="work" href="${w.href}">
        <video class="workPreview" autoplay loop muted playsinline preload="metadata" src="${w.preview}" poster="${w.poster || ""}"></video>
        <div class="workBody">
  ...
  function setIcon(on) {
    if (!audioIcon) return;
    audioIcon.innerHTML = on ? `<path d="M7 5h4v14H7zM13 5h4v14h-4z"></path>` : `<path d="M8 5v14l12-7z"></path>`;
  }
  ...
  async function startAudio() {
    ...
    isOn = true;
    setIcon(true);
    const v = clamp(Number(vol?.value ?? 0.10), 0, 1); // safe fade-in
